...
def main():
    ...
    n_steps = 60  # Number of future steps to predict
    future_predictions = predict_future_prices(model, scaled_new_close_prices, n_steps, seq_length)
    ...
    future_dates = pd.date_range(start=new_data.index[-1], periods=n_steps, freq='T') # When predicting for 24H use - freq='T' -
    future_df = pd.DataFrame(data=future_predictions, index=future_dates, columns=['Pclearedicted Close'])
